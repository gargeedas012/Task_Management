...
export const GetTeamMembersAsync = async (projectId: string, memberId: string): Promise<ApiResponse<TeamMember1[]>> => {
    const response = await api.get("Project/GetTeamMembersAsync", { params: { projectId: projectId, memberId: memberId } });
    return response.data;
}
